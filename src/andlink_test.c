...
void opencpu_andlink_read_cb_error(int mid, int objid, int insid, int resid)
{
    //TODO
    //...
    opencpu_andlink_result(mid, RESULT_400_BADREQUEST, 0);//返回表示该操作结果错误
}
...
void opencpu_andlink_read_cb_ok(int mid, int objid, int insid, int resid)
{
    ...
    if(objid == 19) 
    {
        opencpu_andlink_read(mid, objid, insid, resid, 1, strread, 1);//返回表示该操作结果数据,示例资源类型为string型
    }
    else
    {
        opencpu_andlink_result(mid, RESULT_400_BADREQUEST, 0);//返回表示该操作结果错误
...
void opencpu_andlink_write_cb(int mid, int objid,int insid, int resid, int type, int flag, int len, char *data)
{
    ...
    opencpu_andlink_result(mid, RESULT_204_CHANGED, 0);//操作正确完成返回204
}
...
void test_andlink()
{
	...
	opencpu_andlink_create("lwm.home.komect.com", 13, &callback);//创建设备
	opencpu_andlink_config(device_id);
   	opencpu_andlink_add_obj(19, 1, "1", 0, 0);
    cot_printf("open andlink");
	if(opencpu_andlink_open(30, 86400) == 0)
    {
        ...
        opencpu_andlink_notify(19, 1, 0, 1, strnotify, 1, 0, 0);
